...
public class TileMatrixReconstructor extends TileEntity implements ISidedInventory, ITileHexEnergyDrain {
    ...
    public void recheckSources() {
        scanSources();
        if (HexConfig.cfgGeneralMachineNetworkDebug && HexConfig.cfgGeneralNetworkDebug)
            System.out.println("[Matrix Reconstructor] (" + xCoord + ", " + yCoord + ", " + zCoord + "): Recheck requested. s: " + usableSources);
    }
    ...
    private boolean canProcess() {
        if (this.inventory[0] == null)
            ...
            ItemStack itemStack = HexProcessingMatrixReconstructor.processing().getProcessingResult(inventory[0]);
            if (itemStack == null)
