...
public class TileEnergyNodePortLU extends TileEntity implements ITileHexEnergyPort, ITileEntityEnergy {
    ...
    public void readFromNBT(NBTTagCompound tagCompound) {
        ...
        energyBufferTotal = HexEnergyNode.parseEnergyPerTick(portType, portTier) * 4;
    }
    ...
    public void setPortTier(int portTier) {
        ...
        this.energyBufferTotal = HexEnergyNode.parseEnergyPerTick(this.portType, this.portTier) * 4;
    ...
    public void displayInfoPort(EntityPlayer player) {
        ...
        if (!player.isSneaking()) {
            ...
            player.addChatMessage(new ChatComponentTranslation("msg.probeEnergy.txt",  Math.round(energyBufferFilled), "LU",
                    Math.round(energyBufferTotal), "LU"));
            ...
            if (isPart && linkedPort != null && mode == HexEnergyNode.PORT_MODE_INPUT) {
                ...
                player.addChatMessage(new ChatComponentTranslation("msg.probeEfficiency.txt", (portTier + 1),
                        Math.round((1 - HexEnergyNode.parseEfficiencyMultiplier(portTier, port.getPortTier())) * 100), 0, "LU"));
            }
                ...
                player.addChatMessage(new ChatComponentTranslation("msg.probeEfficiency.txt", (portTier + 1),
                        ...
                        Math.round(HexEnergyNode.parseEnergyPerTick(portType, portTier)), "LU"));
            }
            else if (isPart)
    ...
    private boolean isFormedOutput() {
        return HexUtils.getMetaBit(HexBlocks.META_STRUCTURE_IS_PART, worldObj, xCoord, yCoord, zCoord) &&
                HexUtils.getMetaBitBiInt(HexEnergyNode.META_MODE_0, HexEnergyNode.META_MODE_1, worldObj, xCoord, yCoord, zCoord) == HexEnergyNode.PORT_MODE_OUTPUT;
    }
}
